...
const CONTACT_INFO = [
  ...
  { icon: MapPin, label: "Office", value: "E 279, Industrial Area, Sector 75\nSahibzada Ajit Singh Nagar\nPunjab 160055, India", href: null, type: "text" as const },
];
...
export default function ContactPageClient() {
  ...
  function fieldError(name: string, value: string) {
  ...
  useEffect(() => {
    if (countryOpen && searchInputRef.current) searchInputRef.current.focus();
  }, [countryOpen]);
  ...
  const inputClass = (hasError?: boolean) =>
    `w-full px-4 py-3.5 rounded-xl bg-white border text-[#0D0D1A] placeholder-gray-400 text-sm focus:outline-none focus:ring-2 transition-all duration-200 ${
      hasError
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-[#F8F9FC] pt-20 pb-12 lg:pt-[13rem] lg:pb-12">
        ...
        <div className="container-main relative">
          ...
          >
            <span className="font-light text-[#9CA3AF]">We Build, You{" "}</span>
      ...
      {/* ═══════════════════ MAP + FORM ═══════════════════ */}
      <section id="contact-form" className="bg-white section-pad">
        <div className="container-main">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-[640px]">
          ...
          <motion.div
            ...
            className="relative order-2 lg:order-1 min-h-[380px] lg:min-h-full rounded-2xl overflow-hidden border border-gray-100"
          >
            <iframe
              title="Unitaspro Office"
              src="https://maps.google.com/maps?q=BigBang+Sector+75+SAS+Nagar+Mohali&t=&z=17&ie=UTF8&iwloc=&output=embed"
          ...
          <motion.div
            ...
            className="order-1 lg:order-2 bg-[#F7F8FC] rounded-2xl px-8 py-12 lg:px-12 lg:py-14 flex flex-col justify-center"
          >
              {sent ? (
                <div className="min-h-[500px] flex flex-col items-center justify-center gap-5 p-12 rounded-2xl border border-gray-100 bg-[#F7F8FC] text-center">
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                  ...
                  <div>
                    <h2 className="font-bold text-[#0D0D1A] text-2xl mb-2">Message received!</h2>
                    <p className="text-[#6B7180] text-base">We&apos;ll review your project details and get back within 24 hours.</p>
                  </div>
                  <button className="btn-primary group mt-2" onClick={() => setSent(false)}>
                    Send Another
                    <span className="btn-arrow"><ArrowRight size={15} /></span>
                  </button>
                </div>
                ...
                <div>
                  <div className="mb-8">
                    <h3 className="font-bold text-[#0D0D1A] text-xl mb-1.5">Tell us about your project</h3>
                    <p className="text-[#6B7180] text-sm">Fill in the details and we&apos;ll get back within 24 hours with a free consultation.</p>
                  </div>

                  <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Full Name *</label>
                        <input required name="name" type="text" value={form.name} onChange={handleChange} onBlur={handleBlur} placeholder="John Smith" className={inputClass(!!fieldError("name", form.name))} />
                        {fieldError("name", form.name) && <p className="text-xs text-red-500 mt-0.5 pl-1">{fieldError("name", form.name)}</p>}
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Email *</label>
                        <input required name="email" type="email" value={form.email} onChange={handleChange} onBlur={handleBlur} placeholder="john@company.com" className={inputClass(!!fieldError("email", form.email))} />
                        {fieldError("email", form.email) && <p className="text-xs text-red-500 mt-0.5 pl-1">{fieldError("email", form.email)}</p>}
                      </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Phone / WhatsApp</label>
                        <div className="flex gap-0">
                            ...
                            <AnimatePresence>
                              {countryOpen && (
                                <motion.div
                                  ...
                                  className="absolute top-full left-0 mt-1 w-64 bg-white rounded-xl border border-gray-200 shadow-[0_8px_32px_rgba(15,23,42,0.12)] z-50 overflow-hidden"
                                >
                                  <div className="p-2 border-b border-gray-100">
                                    <input
                                      ...
                                      className="w-full px-3 py-2 rounded-lg bg-[#F7F8FC] border-0 text-sm text-[#0D0D1A] placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-400/30"
                                    />
                                  </div>
                                    ...
                                    {filteredCountries.length === 0 ? (
                                      ...
                                      filteredCountries.map((c) => (
                                        <button
                                          ...
                                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-left text-sm hover:bg-brand-50 transition-colors duration-100
                          ...
                          <input
                            name="phone" type="tel" inputMode="numeric" maxLength={15}
                            value={form.phone}
                            ...
                            className="flex-1 min-w-0 px-4 py-3.5 rounded-r-xl bg-white border border-gray-200 text-[#0D0D1A]
                              placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all duration-200"
                          />
                        </div>
                      </div>
                      <div className="flex flex-col gap-1.5">
                        <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Service Needed</label>
                        <select name="service" value={form.service} onChange={handleChange} className={`${inputClass()} pr-8`}>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Message</label>
                      <textarea
                    ...
                    {/* Captcha */}
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-[#374151] uppercase tracking-[0.18em]">Verify You&apos;re Human</label>
                      <div className="flex items-center gap-4">
                        ...
                        <input
                          ...
                          placeholder="Answer"
                          maxLength={3}
                          className={`w-28 h-[50px] px-4 rounded-xl bg-white border text-[#0D0D1A] placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all duration-200
                            ${captchaError ? "border-red-400 bg-red-50" : "border-gray-200"}`}
                        />
                        {captchaError && (
                          <span className="text-xs text-red-500">Incorrect, try again</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        disabled={loading || !form.name.trim() || !form.email.trim() || !captchaInput.trim()}
      ...
      <section className="bg-[#F7F8FC] border-t border-gray-100">
        <div className="container-main py-16 lg:py-20">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="heading-xl text-[#0D0D1A] mb-3">Before You Reach Out</h2>
            <p className="text-[#6B7180] text-lg">Learn more about our timelines, pricing, and international delivery standards.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { q: "How Quickly Do You Respond?", a: "We guarantee a professional technical response within 24 business hours." },
              { q: "Is The Consultation Really Free?", a: "Yes, our initial 30-minute strategy session is complimentary with no commitment." },
              { q: "What Information Should I Include?", a: "Please provide your project goals, preferred timeline, and estimated budget range." },
              { q: "Do You Work With International Clients?", a: "We currently serve partners in the UAE, Canada, UK, and India." },
              { q: "What Happens After I Send The Form?", a: "Our engineers review your brief and schedule a discovery call session." },
              { q: "Do You Sign NDA?", a: "We provide signed NDA documents to protect your intellectual property immediately." },
            ].map((faq, i) => (
              <motion.div
                key={faq.q}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="group/step relative bg-white rounded-2xl p-6 border border-gray-100/80
                  shadow-[0_1px_8px_rgba(15,23,42,0.05)] hover:shadow-[0_8px_32px_rgba(15,23,42,0.10)]
          ...
          >
            <p className="text-[#6B7180] text-sm mb-4">Still have questions?</p>
            <div className="flex items-center justify-center gap-3">
              <Link href="#contact-form" className="btn-primary group">
                Get Free Quote
                <span className="btn-arrow"><ArrowRight size={15} /></span>
              </Link>
              <a href="tel:+918264954344" className="btn-secondary">Call us now</a>
            </div>
          </motion.div>
        </div>
      </section>
